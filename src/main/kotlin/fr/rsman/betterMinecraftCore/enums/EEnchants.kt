...
import fr.rsman.betterMinecraftCore.enchantments.Protection
import fr.rsman.betterMinecraftCore.enchantments.Telekinesis
import fr.rsman.betterMinecraftCore.enchantments.Aiming
...
enum class EEnchants(val key: String, val enchant: Enchantment, val replaces: List<EEnchants>? = null) {
    ...
    companion object {
        ...
        val nonReplacedKeys: List<String>
           get() = keys.toMutableList().apply { removeAll(values().mapNotNull { it.replaces }.flatten().map { it.key }.toSet()) }
